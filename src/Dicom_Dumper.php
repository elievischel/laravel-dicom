...
use elievischel\laravelDicom\dict\Nanodicom;
use elievischel\laravelDicom\Nanodicom_Dictionary;
use function elievischel\laravelDicom\phpsprintf;
use const elievischel\laravelDicom\dumper;
...
class Dicom_Dumper extends Nanodicom
{
    ...
    protected function _print_element($element)
    {
        ...
        foreach ($this->output['columns'] as $column => $values) {
            switch ($column) {
                case 'name' :
                    $string = isset(Nanodicom_Dictionary::$dict[$element['g']][$element['e']])
