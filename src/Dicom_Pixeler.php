...
use elievischel\laravelDicom\Gmagick;
use elievischel\laravelDicom\GmagickDraw;
use elievischel\laravelDicom\GmagickPixel;
use elievischel\laravelDicom\Imagick;
use elievischel\laravelDicom\ImagickDraw;
use elievischel\laravelDicom\ImagickPixel;
use elievischel\laravelDicom\integet;
use elievischel\laravelDicom\Nanodicom_Exception;
use function elievischel\laravelDicom\phprealpath;
use function elievischel\laravelDicom\phpsubstr;
use const elievischel\laravelDicom\pixeler;
...
class Dicom_Pixeler extends Nanodicom
{
    ...
    public static function create_image($cols, $rows)
    {
        switch (self::$driver) {
            case 'gd':
                return imagecreatetruecolor($cols, $rows);
            case 'gmagick':
                $image = new Gmagick();
    ...
    protected function _allocate_color_rgb($image, $rgb)
    {
        switch (self::$driver) {
            ...
            case 'gmagick':
                return new GmagickPixel('rgb(' . $rgb[0] . ',' . $rgb[1] . ',' . $rgb[2] . ')');
            case 'imagick':
                return new ImagickPixel('rgb(' . $rgb[0] . ',' . $rgb[1] . ',' . $rgb[2] . ')');
        }
    ...
    protected function _allocate_color_gray($image, $gray)
    {
        switch (self::$driver) {
            ...
            case 'gmagick':
                return new GmagickPixel('rgb(' . $gray . ',' . $gray . ',' . $gray . ')');
            case 'imagick':
                return new ImagickPixel('rgb(' . $gray . ',' . $gray . ',' . $gray . ')');
        }
    ...
    public function write_image($image, $location, $format = 'jpg')
    {
        switch (self::$driver) {
            case 'gd' :
                switch ($format) {
                    case 'png':
                        imagepng($image, $location . '.png');
                        break;
                    case 'gif':
                        imagegif($image, $location . '.gif');
                        break;
                    case 'jpg':
                    default:
